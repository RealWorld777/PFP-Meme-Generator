        ...
        <div className="border-t-2 border-black p-10">
          {faqData.map((faq, index) => (
            <div key={index} className="flex justify-between p-5 border-2 border-black mb-5">
              <div>
                <h3 className="bricolageSemibold text-xl sm:text-4xl">{faq.title}</h3>
                {activeIndex === index && <p className="workSans sm:text-2xl mt-5 faq-content active">{faq.content}</p>}
              </div>
